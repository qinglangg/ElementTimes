...
import com.elementtimes.tutorial.common.block.pipeline.ItemElement;
 ...
 * @author luqin
 */
public class ItemInputPipeline extends BaseItemPipeline {
    ...
    public void tick() {
        ...
        for (EnumFacing direction : EnumFacing.values()) {
            ...
            if (te != null) {
                IItemHandler handler = te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, direction.getOpposite());
                if (handler != null) {
                    for (int i = 0; i < handler.getSlots(); i++) {
                        ItemStack extract = handler.extractItem(i, handler.getSlotLimit(i), true);
                        if (!extract.isEmpty()) {
                            BaseElement element = ItemElement.TYPE.newInstance();
                            element.element = extract.copy();
                            if (element.send(world, pos, offset)) {
                                handler.extractItem(i, extract.getCount(), false);
                                break;
                            }
                        }
                    }
