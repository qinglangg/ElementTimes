...
public final class Wire extends Item {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
        ...
        if (!itemstack.isEmpty() && player.canPlayerEdit(blockPos, facing, itemstack)) {
            ...
            if (iblockstate1 != null) {
	            ...
	            List<BlockPos> update = (List<BlockPos>) os[1];
	            for (BlockPos bp : update) {
	 ...
	 * {@link #whatState(World, IBlockState, BlockPos, BlockPos[], boolean, BlockPos[], TileEntityWire)}
	...
	public static void updateBlock(World world, BlockPos pos, BlockPos[] donnotLink) {
		TileEntityWire nbt = (TileEntityWire) world.getTileEntity(pos);
		if (nbt == null) {
	...
	/**
	 * 该方法时{@link #whatState(World, IBlockState, BlockPos, BlockPos[], boolean, BlockPos[], TileEntityWire)}方法的转接，
	...
	private static Object[] whatState(World world, Block block, BlockPos pos, BlockPos[] firstPos) {
		return whatState(world, block.getDefaultState(), pos, null, false, firstPos, null);
	}
	...
	private static Object[] whatState(World world, IBlockState state, BlockPos pos,
	                                  BlockPos[] donnotLink, boolean isExisting, BlockPos[] firstPos, TileEntityWire nbt) {
		//准备设置的TE
		boolean isExistNow;
		if (nbt == null) {
			nbt = new TileEntityWire();
