...
public class DefaultMachineLifecycle implements IMachineLifeCycle {
    ...
    public DefaultMachineLifecycle(BaseMachine machine) {
    ...
    public void onStart() {
        ...
        machine.setEnergyUnprocessed(Math.abs(recipe.energy.applyAsInt(recipe)));
        // items
    ...
    public boolean onCheckResume() {
        ...
        assert recipe != null;
        int change = Math.min(machine.getMaxEnergyChange(), machine.getEnergyUnprocessed());
        ...
        for (int i = 0; pushAll && i < itemCount; i++) {
            ...
            for (int j = 0; j < outputItems.getSlots(); j++) {
                ...
                if (!slot.isEmpty() && slot.isItemEqual(left)) {
                    left = outputItems.insertItem(j, left, simulate);
                }
            }
            // 空槽位
            for (int j = 0; j < outputItems.getSlots(); j++) {
