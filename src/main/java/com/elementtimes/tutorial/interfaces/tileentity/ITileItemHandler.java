 ...
 * @author luqin2007
 */
public interface ITileItemHandler<T extends IItemHandler & INBTSerializable<NBTTagCompound>> extends ICapabilityProvider, INBTSerializable<NBTTagCompound> {

    Map<EnumFacing, SideHandlerType> getItemTypeMap();
    Map<SideHandlerType, T> getItemHandlerMap();
    ...
    @Nonnull
    default T getItemHandler(@Nonnull SideHandlerType type) {
        return getItemHandlerMap().get(type);
    }

    @Nonnull
    default T getItemHandler(EnumFacing facing) {
    ...
    default NBTTagCompound writeToNBT(NBTTagCompound nbtTagCompound) {
        ...
        // input
        T inputs = getItemHandler(SideHandlerType.INPUT);
        if (inputs.getSlots() != 0) {
            nbt.setTag(NBT_ITEMS_INPUT, inputs.serializeNBT());
        }
        // output
        T outputs = getItemHandler(SideHandlerType.OUTPUT);
