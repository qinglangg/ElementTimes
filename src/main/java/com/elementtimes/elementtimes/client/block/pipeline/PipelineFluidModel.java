...
public class PipelineFluidModel implements IUnbakedModel {

    public static final ModelResourceLocation MODEL_FLUID_CORE = new ModelResourceLocation(ElementTimes.MODID, "pipelinefluidconnect");
    ...
    public Collection<ResourceLocation> getDependencies() {
        return Arrays.asList(MODEL_FLUID_CONNECT, MODEL_FLUID_INPUT, MODEL_FLUID_OUTPUT);
    }

    @Override
    @Nonnull
    public Collection<ResourceLocation> getTextures(Function<ResourceLocation, IUnbakedModel> modelGetter, Set<String> missingTextureErrors) {
        return Arrays.asList(TEXTURE_FLUID_CORE, TEXTURE_FLUID_IO);
    }
    ...
    public IBakedModel bake(ModelBakery bakery, Function<ResourceLocation, TextureAtlasSprite> spriteGetter, ISprite sprite, VertexFormat format) {
        try {
            IBakedModel model = ModelLoaderRegistry.getModel(MODEL_FLUID_CORE).bake(bakery, spriteGetter, sprite, format);

            return new BakedPipelineModel(model, "", ((direction, state) -> {
                if (state.getBlock() == Industry.pipelineFluidConnect) {
