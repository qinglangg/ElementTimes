...
public class FuelGeneratorRecipeLifecycle extends BaseRecipeLifecycle {

    private final TileFuelGenerator mTile;
    ...
    public FuelGeneratorRecipeLifecycle(TileFuelGenerator tile) {
        mTile = tile;
    }

    @Override
    protected void inputItem(RecipeCapture capture) {
        ItemStack input = mTile.getItemHandler().extractItemUncheck(0, 1, false);
        if (input.hasContainerItem()) {
            ItemStack item = input.getContainerItem();
            mTile.getItemHandler().insertItemUncheck(0, item, false);
        }
    }
    ...
    protected Optional<RecipeCapture> getNextRecipe() {
        ItemStack input = mTile.getItemHandler().extractItemUncheck(0, 1, true);
        int burnTime = ForgeHooks.getBurnTime(input);
        if (burnTime > 0) {
            int energy = (int) (burnTime * Config.genFuelMultiple.get());
            RecipeCapture capture = new RecipeCapture(NonNullList.withSize(1, input), NonNullList.create(), NonNullList.create(), NonNullList.create(), -energy, 0);
    ...
    protected boolean isRecipeCanWork(@Nonnull RecipeCapture capture) {
        if (mTile.getItemHandler().extractItemUncheck(0, 1, true).isItemEqual(capture.getItemInputs().get(0))) {
    ...
    protected int getEnergySpeed() {
        return Config.genElemSpeed.get() * getInterval();
    }
