 ...
 * 若{@link TileEntity#isRemoved()}返回true，系统会自动取消该TE的注册
 * @author EmptyDreams
 * @version V1.1
 */
public final class NetworkRegister {
	...
	static void forEach(boolean isClient, Consumer<? super IAutoNetwork> consumer) {
		registerAll();
		handleNetworkEntity(isClient ? NETWORKS_CLIENT : NETWORKS_SERVICE, consumer);
...
//		}
	...
	private static void handleNetworkEntity(List<IAutoNetwork> list, Consumer<? super IAutoNetwork> consumer) {
		Iterator<IAutoNetwork> iterator = list.iterator();
		while (iterator.hasNext()) {
			IAutoNetwork network = iterator.next();
			TileEntity entity = (TileEntity) network;
			if (entity.isRemoved()) {
				iterator.remove();
				continue;
			}
			World world = entity.getWorld();
			if (world != null && world.isAreaLoaded(entity.getPos(), 1)) {
				consumer.accept(network);
			}
		}
	}
	...
	public static synchronized void register(IAutoNetwork net) {
		if (!(net instanceof TileEntity)) {
			throw new ClassCastException("'net' is not extends TileEntity!");
		}
