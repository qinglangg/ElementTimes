...
public final class EleWorker {
	...
	public static void registerOutputer(IEleOutputer outputer) {
		Objects.requireNonNull(outputer, "outputer is null");
	...
	public static void registerInputer(IEleInputer inputer) {
		Objects.requireNonNull(inputer, "inputer is null");
	...
	public static void registerTransfer(IEleTransfer transfer) {
		Objects.requireNonNull(transfer, "transfer is null");
	...
	public static UseInfo useEleEnergy(TileEntity te) {
		...
		System.err.println("该方块没有找到可用的托管：" + te);
		return null;
	}
	...
	public static UseInfo useEleEnergy(TileEntity te, IEleInputer inputer) {
		...
		if (outs.isEmpty()) {
			...
			for (Map.Entry<TileEntity, IEleTransfer> entry : transfers.entrySet()) {
				...
				if (pathInfo == null || pathInfo.getOuter() == null) {
					continue;
				}
